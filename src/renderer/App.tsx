...
export default function App() {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        let base64 = ''
        for (let j = 0; j < bytes.length; j++) base64 += String.fromCharCode(bytes[j])
        base64 = btoa(base64)
        // Copy to workspace
        const destPath = `${workspacePath}/${file.name}`
        const content = new TextDecoder().decode(buffer)
        await window.electronAPI.writeTextFile(destPath, content)
      } catch { /* ignore */ }
    }
  ...
  const handleDiffAccept = useCallback(async () => {
    ...
    } else if (target === 'document' && selectedFilePath) {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden"
      ...
      {/* Top-right buttons */}
      <div className="fixed top-3 right-3 flex items-center gap-1.5 z-40" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        <button onClick={toggleTheme} className="w-7 h-7 rounded-full flex items-center justify-center smooth-transition glass"
          style={{ color: 'var(--text-secondary)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
        ...
        <button onClick={() => { setShowSettings(true); setSettingsMsg(null) }} className="w-7 h-7 rounded-full flex items-center justify-center smooth-transition glass"
          style={{ color: hasApiKey() ? 'var(--accent)' : 'var(--text-secondary)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }} title="设置">
      ...
      {pendingDiff && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 w-[700px] max-w-[90vw] shadow-2xl">
          <DiffPanel oldContent={pendingDiff.oldContent} newContent={pendingDiff.newContent} title={pendingDiff.title} onAccept={handleDiffAccept} onReject={handleDiffReject} />
        </div>
      )}

      {/* Settings Modal - abbreviated, same as before */}
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center animate-fade-in"
          ...
          <div className="w-[520px] rounded-2xl p-6 shadow-lg max-h-[85vh] overflow-y-auto"
            ...
            <div className="flex items-center justify-between mb-4">
              <div><h2 className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>选择 AI 服务</h2>
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-tertiary)' }}>选择一个提供商并填入 API Key 即可开始使用</p></div>
              <button onClick={() => setShowSettings(false)} className="w-6 h-6 rounded-full flex items-center justify-center smooth-transition" style={{ color: 'var(--text-tertiary)' }}>
            ...
            {PROVIDERS[activeProvider].requiresAuth && (
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  {keyConfigured && <button onClick={handleClearKey} className="px-3 py-2 rounded-lg text-xs smooth-transition" style={{ color: '#ef4444', background: 'rgba(239,68,68,0.1)' }}>清除</button>}
                </div>
              </div>
            )}

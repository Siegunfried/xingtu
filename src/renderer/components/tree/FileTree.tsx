...
export default function FileTree() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      useWorkspaceStore.getState().setWorkspacePath(saved)
    }
  }, [])
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center">
            ...
            <button onClick={openWorkspace}
              className="text-xs px-4 py-2 rounded-full smooth-transition"
  ...
  const isDoc = (entry: FileEntry) => {
    const ext = entry.name.split('.').pop()?.toLowerCase() || ''
    return DOC_EXTS.has(ext)
  }

  const isNoteDir = (entry: FileEntry) => {
    // Note dirs are directories named after a document
    return entry.isDirectory && rootEntries.some(
      (e) => !e.isDirectory && e.name.replace(/\.[^.]+$/, '') === entry.name
    )
  }
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-secondary)' }}>
      ...
      <div className="flex items-center justify-between px-3 py-2.5 flex-shrink-0"
        ...
        <button onClick={openWorkspace} className="w-6 h-6 rounded flex items-center justify-center hover:opacity-70"
...
function TreeItem({
  ...
  return (
    <div className="animate-slide-in">
      ...
      {/* Sub items for expanded directories */}
      {isDir && isExpanded && subEntries.map((sub) => (
        <div key={sub.path} className="ml-4" onClick={(e) => e.stopPropagation()}>
          <div
